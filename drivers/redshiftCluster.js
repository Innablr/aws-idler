...
class InstrumentedRedshiftCluster extends ToolingInterface {
    constructor(awsResource, region, accountId) {
        super(awsResource);
        this.clusterArn = `arn:aws:redshift:${region}:${accountId}:cluster:${this.resourceId}`;
        this.tags = [];
    }
    ...
    get isSingleNode() {
        return Boolean(this.singleNode);
    }
...
class RedshiftClusterDriver extends DriverInterface {
    ...
    stopOneCluster(cluster) {
        ...
        const snapshotId = `idler-cluster-${cluster.resourceId}-${locaTimeNow.format('YYYYMMDDhhmmss')}`;
        ...
        const preserveTags = cluster.resource.Tags
            .concat([
                {Key: 'idler/cluster_subnet_group_name', Value: cluster.resource.ClusterSubnetGroupName},
                {Key: 'idler/vpc_security_groups', Value: cluster.resource.VpcSecurityGroups.map(xr => xr.VpcSecurityGroupId).join('/')},
                {Key: 'idler/db_name', Value: cluster.resource.DBName || 'dev'},
                {Key: 'idler/vpc_id', Value: cluster.resource.VpcId},
                {Key: 'idler/availability_zone', Value: cluster.resource.AvailabilityZone},
                {Key: 'idler/node_type', Value: cluster.resource.NodeType},
                {Key: 'idler/nodes_number', Value: cluster.resource.NumberOfNodes.toString()},
                {Key: 'idler/cluster_port', Value: cluster.resource.Endpoint.Port.toString()}
            ]);
    ...
    async collect() {
        ...
        const redshiftClusters = await redshift.describeClusters({}).promise()
            .then(c => c.Clusters);

        logger.info('Found %d Redshift clusters', redshiftClusters.length);

        return redshiftClusters.map(cluster => new InstrumentedRedshiftCluster(cluster, this.accountConfig.region, this.Id));
    }
}
