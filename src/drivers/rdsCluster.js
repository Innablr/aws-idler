...
class RdsClusterDriver extends DriverInterface {
    ...
    stopOneCluster(cluster) {
        ...
        const preserveTags = cluster.writerInstance.tags
            .concat([
                {Key: 'idler/db_subnet_group_name', Value: cluster.resource.DBSubnetGroup.DBSubnetGroupName},
                {Key: 'idler/db_security_groups', Value: cluster.resource.VpcSecurityGroups.map(xr => xr.VpcSecurityGroupId).join('/')},
                {Key: 'idler/cluster_port', Value: cluster.resource.Port.toString()},
                {Key: 'idler/db_name', Value: cluster.resource.DatabaseName}
            ])
            .concat(cluster.instances.map(xs =>
                ({
                    Key: `idler/instance/${xs.instanceData.DBInstanceIdentifier}`,
