...
class RdsMultiAzDriver extends DriverInterface {
    ...
    stopOneInstance(instance) {
        ...
        const snapshotId = `idler-multiaz-${instance.resourceId}-${locaTimeNow.format('YYYYMMDDhhmmss')}`;
        ...
        const preserveTags = instance.tags.concat([
            {Key: 'idler/db_subnet_group_name', Value: instance.resource.DBSubnetGroup.DBSubnetGroupName},
            {Key: 'idler/db_security_groups', Value: instance.resource.VpcSecurityGroups.map(xr => xr.VpcSecurityGroupId).join('/')}
        ]);
