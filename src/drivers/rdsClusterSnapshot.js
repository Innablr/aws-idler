...
class RdsClusterSnapshotDriver extends DriverInterface {
        ...
        return assume.connectTo(this.accountConfig.assumeRoleArn)
            ...
            .then(function(rds) {
                return Promise.all(resources.map(function(xs) {
                    const sg = xs.tag('idler/db_security_groups');
                    if (sg === undefined) {
    ...
    startOneSnapshot(snapshot) {
        ...
        return assume.connectTo(this.accountConfig.assumeRoleArn)
            ...
            .then(function() {
                ...
                const sgTag = snapshot.tag('idler/db_security_groups');
                const opts = {
                    ...
                    DBSubnetGroupName: snapshot.tag('idler/db_subnet_group_name'),
                    DatabaseName: snapshot.tag('idler/db_name'),
            ...
            .then(function() {
                return Promise.all(snapshot.tags.filter(xt => xt.Key.startsWith('idler/instance'))
                    .map(function(xt) {
                        ...
                        return {
                            ...
                            DBSubnetGroupName: snapshot.tag('idler/db_subnet_group_name'),
                            EngineVersion: snapshot.resource.EngineVersion,
                            Tags: snapshot.tags.filter(xxt => !xxt.Key.startsWith('idler/'))
                        };
                    })
                    .map(function(xo) {
    ...
    maskstart(resource) {
        if (resource.tag('idler/restore_commenced') !== undefined) {
            return `RDS snapshot ${resource.resource.resourceId} already started restoring at ${resource.tag('idler/restore_commenced')}`;
        }
    }
        ...
        return assume.connectTo(this.accountConfig.assumeRoleArn)
            ...
            .then(function(r) {
                const clusterSnapshots = r.DBClusterSnapshots
                    .filter(function(xs) {
                        if (! (/^idler-cluster-/).test(xs.DBClusterSnapshotIdentifier)) {
                            logger.info('RDS snapshot %s is not created by Idler, skipping', xs.DBClusterSnapshotIdentifier);
                            return false;
                        }
                        return true;
                    })
