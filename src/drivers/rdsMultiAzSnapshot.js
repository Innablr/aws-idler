...
class RdsSnapshotDriver extends DriverInterface {
        ...
        return assume.connectTo(this.accountConfig.assumeRoleArn)
            ...
            .then(function() {
                ...
                return rds.restoreDBInstanceFromDBSnapshot({
                    ...
                    MultiAZ: true,
                    Tags: snapshot.tags.filter(xt => !xt.Key.startsWith('idler/'))
                }).promise();
            })
            .then(function() {
                ...
                return rds.addTagsToResource({
                    ...
                    Tags: [{Key: 'idler/restore_commenced', Value: locaTimeNow.format('YYYYMMDDhhmmss')}]
                }).promise();
            })
            .catch(function(err) {
    ...
    start(resources) {
        const that = this;
        return Promise.all(resources.map(function(xs) {
            if (xs.tag('idler/restore_commenced') !== undefined) {
                that.logger.info('RDS snapshot %s already started restoring at %s', xs.resourceId, xs.tag('idler/restore_commenced'));
                return Promise.resolve();
            }
        ...
        return assume.connectTo(this.accountConfig.assumeRoleArn)
            ...
            .then(function(r) {
                const dbSnapshots = r.DBSnapshots
                    .filter(function(xs) {
                        if (! (/^idler-multiaz-/).test(xs.DBSnapshotIdentifier)) {
                            logger.info('RDS snapshot %s is not created by Idler, skipping', xs.DBSnapshotIdentifier);
                            return false;
                        }
                        return true;
                    })
