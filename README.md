Idler, an AWS powercycle facility and more
======
...
Idler does exactly that.

Idler:
 * is run in AWS Lambda, sends log in Cloudwatch Logs
 ...
 * has a responsibilty separation between drivers that directly operate the resources and plugins that work out what needs to be done with a resource, which gives Idler an amazing extensibility potential

Apart from powercycling resources Idler can validate that required tags are set on AWS resources and their values match a regular expression.


What
------

Idler currently supports the following AWS resources:
 * EC2 instances including those run in Autoscaling, Idler will pause the ASG prior to shutting down instances
 * EBS volumes. Idler will tag these resources. Idler can get the tags from the parent instance/volume.
 * Snapshots. Idler will tag these resources. Idler can get the tags from the parent instance/volume.
 * RDS single instances, Idler will use the native start/stop featur
 * RDS multi-az instances, Idler will snapshot the instance and delete it. In the morning it will be restored from snapshot
 * RDS Clusters, Idler will snapshot the cluster and save the critical information about the cluster members on the snapshot

Idler does not support RDS instances with read replicas, as it is very difficult to ensure integrity for such configurations
...
This repository only contains the Idler code and no deployment mechanisms. The make target `bundle` produces a zip-file
suitable for deployment in AWS Lambda.

You can use [cloudformation-seed](https://github.com/Innablr/cloudformation-seed) for deployment

### Configure

Idler configuration is done in YAML. First line in the config file must be `---` as per YAML specification.
...
1. Section `defaults` defines default behavior for all accounts. Settings in this section will be overriden in the accounts section
    ...
    | timezone_tag | Idler will read this tag on individual resources to override account-wide timezone | Timezone |
    | organization_role_name | Role to be assumed on the main account from organizations to get the accounts list from it | - |
    | idler_role_name | Idler role name to be assumed on each client account | - |
...
There is also special values for the schedule tag:
  * `24x7` - resource will be always up. If you stop it manually Idler will attempt to bring it up
  * `24x5` - resource will be always up except for Saturday and Sunday
  * `0x7` - resource will be always down. If you manually start it Idler will bring it down
...
This is a sidekick plugin for `rdsMultiAzSnapshot`. When `rdsMultiAzSnapshot` restores the database from a snapshot the security groups on the database can only be set after the restore is fully complete. So Idler will attempt setting the security groups on its subsequent runs by running the snapshot through this plugin.
...
|allow_set_from_parent|Allow Idler to try to get tags from parent (instance/volumes) -- works only with ebs/snapshot drivers|`true`,`false`|`true`|
...
plugins:
  - name: validateTags
    tag: [ CostCentre, OwnerEmail, Service ]
    tag_missing:
      - warn
      - stop
    tag_not_match:
